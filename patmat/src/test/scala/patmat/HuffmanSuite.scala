...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("times of 'a' in List('a', 'b', 'a', 'c')") {
    assertEquals(
      times(List('a', 'b', 'a', 'c')),
      List(('a', 2), ('b', 1), ('c', 1))
    )
  ...
  test("check if list of code trees contains only one code tree") {
    new TestTrees:
      assertEquals(singleton(List(Leaf(1, 2))), true)
      assertNotEquals(singleton(List(t1, t2)), true)
  }
  ...
  test("combine List[CodeTree] until it isn't a singleton") {
    new TestTrees:
      val l = until(singleton, combine)(List(t1, t2, Leaf('m', 2)))
