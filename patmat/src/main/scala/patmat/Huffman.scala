...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    ...
    for c <- chars do map = map.updated(c, map.getOrElse(c, 0) + 1)
    map.toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.map((c, w) => Leaf(c, w)).sortBy(_.weight)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    def decodeOneChar(
      ...
      tree match
        ...
        case Fork(l, r, _, _) =>
          if bits.head == 1 then decodeOneChar(r, bits.tail)
          else decodeOneChar(l, bits.tail)

    def decodeAcc(chars: List[Char], bits: List[Bit]): List[Char] =
      val (b, c) = decodeOneChar(tree, bits)
      if b.size == 0 then c :: chars
      else decodeAcc(c :: chars, b)

    decodeAcc(List.empty, bits).reverse
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def traversal(tree: CodeTree)(char: Char): List[Bit] = tree match
      case Leaf(_, _) => Nil
      case Fork(left, right, _, _) =>
        if chars(left).contains(char) then 0 :: traversal(left)(char)
        else 1 :: traversal(right)(char)

    text.flatMap(traversal(tree))
  ...
  def convert(tree: CodeTree): CodeTable =
    def traversal(tree: CodeTree, bits: List[Bit]): CodeTable = tree match
      case Leaf(char, _) => List((char, bits.reverse))
      case Fork(left, right, _, _) =>
        traversal(left, 0 :: bits) ::: traversal(right, 1 :: bits)
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable =
    def tableAcc(acc: CodeTable, table: CodeTable): CodeTable =
      if table.size == 0 then acc
      else if acc.contains(table.head) then tableAcc(acc, table.tail)
      else tableAcc(table.head :: acc, table.tail)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    def encodeWithTable(text: List[Char], bits: List[Bit]): List[Bit] =
      if text.size == 0 then bits
      else encodeWithTable(text.tail, bits ::: codeBits(table)(text.head))

    encodeWithTable(text, Nil)
