...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(
    ...
    val questions = postings
      .filter { p =>
        p.postingType == 1 && p.tags.isDefined
      }
      .map(p => (p.id, p))
  ...
  def scoredPostings(
    ...
    grouped.map { case (_, qAs) =>
      val highScore = answerHighScore(qAs.map(_._2).toArray)
      (qAs.head._1, highScore)
    }
  ...
  def vectorPostings(
    ...
    val filtered = scored.filter((question, _) =>
      firstLangInTag(question.tags, langs) match
        case None => false
        case _    => true
    )

    filtered.map((question, highScore) =>
  ...
  def clusterResults(
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langLabel: String =
        val langCounts = vs.map { case (langIndex, _) =>
          (langs(langIndex / langSpread), 1)
        }
        langCounts
          .groupBy(_._1)
          .view
      ...
      val langPercent: Double =
        val dominantLangCount = vs.count { case (langIndex, _) =>
          langs(langIndex / langSpread) == langLabel
        }
        (dominantLangCount.toDouble / vs.size.toDouble) * 100.0

      val clusterSize: Int = vs.size

      val medianScore: Int =
        val sortedScores = vs.map { case (_, score) => score }.toArray.sorted
