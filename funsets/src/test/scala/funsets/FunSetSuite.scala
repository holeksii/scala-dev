...
class FunSetSuite extends munit.FunSuite:
  ...
  test("intersection contains elements from both sets") {
    new TestSets:
      val emptyIntersection = intersect(s1, s2)
      val intersection = intersect(s1, s1)

      assert(!contains(emptyIntersection, 1), "Intersection 1")
      assert(contains(intersection, 1), "Intersection 2")
  }
  ...
  test("diff") {
    new TestSets:
      ...
      val emptyDiffSet = diff(s1, s1)
      assert(!contains(emptyDiffSet, 1))
  }
  ...
  test("map") {
    new TestSets:
      ...
      assert(contains(map(s, p), 2))
      assert(contains(map(s, p), 4))
  }

  test("set") {
    new TestSets:
      val bigSet = set(1, 2, 3, 4)

      assert(contains(bigSet, 1))
      assert(contains(bigSet, 2))
      assert(contains(bigSet, 3))
      assert(contains(bigSet, 4))
      assert(!contains(bigSet, 5))
      assert(!contains(bigSet, 6))
  }
