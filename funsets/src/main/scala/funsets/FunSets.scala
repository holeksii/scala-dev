...
trait FunSets extends FunSetsInterface:
  ...
  def set(elements: Int*): FunSet =
    def iter(acc: FunSet, elements: Seq[Int]): FunSet =
      if elements.isEmpty then acc
      else iter(union(acc, singletonSet(elements.head)), elements.tail)
    iter((x: Int) => false, elements)
