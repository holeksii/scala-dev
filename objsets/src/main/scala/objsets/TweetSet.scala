...
abstract class TweetSet extends TweetSetInterface:
  ...
  def size(set: TweetSet): Int = asSet.size

  override def toString(): String = asSet.toString
  ...
  def descendingByRetweet: TweetList =
    ...
    list
      .sortBy(_.retweets)
      .foldLeft[TweetList](Nil)((acc, tweet) => Cons(tweet, acc))
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  // create new TweetSet with the most retweeted tweet
...
trait TweetList:
  ...
  def tail: TweetList
  def isEmpty: Boolean
  def +=(tweet: Tweet): TweetList =
    if isEmpty then Cons(tweet, Nil)
    else Cons(head, tail += tweet)
